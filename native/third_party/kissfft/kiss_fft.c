...
static void kf_bfly5(kiss_fft_cpx* Fout, const size_t fstride,
    ...
    kiss_fft_cpx ya = tw[fstride*m];
    kiss_fft_cpx yb = tw[fstride*2*m];
    Fout0=Fout; Fout1=Fout0+m; Fout2=Fout0+2*m; Fout3=Fout0+3*m; Fout4=Fout0+4*m;
    for (u=0; u<m; ++u) {
        scratch[0] = *Fout0;
...
static void kf_bfly_generic(kiss_fft_cpx* Fout, const size_t fstride,
    ...
    for (u=0; u<m; ++u) {
        k = u;
        for (q1=0; q1<p; ++q1) { scratch[q1] = Fout[k]; k += m; }
        k = u;
        for (q1=0; q1<p; ++q1) {
            int twidx = 0;
            Fout[k] = scratch[0];
...
kiss_fft_cfg kiss_fft_alloc(int nfft, int inverse_fft, void* mem, size_t* lenmem) {
    ...
    if (nfft >= (int)((SIZE_MAX - 2*sizeof(struct kiss_fft_state))/sizeof(kiss_fft_cpx)))
        return NULL;
    size_t memneeded = KISS_FFT_ALIGN_SIZE_UP(
        sizeof(struct kiss_fft_state) + sizeof(kiss_fft_cpx)*(nfft-1));
    if (lenmem == NULL) {
